<template>
    <div class="html_img grid grid-cols-6">
        <div v-for="item in imgs" class="shadow-md transform-scale-95 bg-white" :id="`dom_img_${item.src}`">
            <Checkbox class="position-absolute pos-right-1 z-1" v-show="isSelect" v-model:checked="item.select" />
            <Image :src="item.src" :fallback="empty" />
        </div>
        <FloatButtonGroup style="right: 180px;bottom: 15px;">
            <Card class="min-w-40 min-h-10 p-2 shadow-xl">
                <div v-show="isSelect">
                    <Checkbox v-model:checked="allSelect">全选</Checkbox>
                    <Button class="mt-2" size="small" type="primary" :icon="h(DownloadOutlined)"
                    @click="onSave">保存</Button>
                </div>
                <Checkbox v-model:checked="isSelect">显示操作</Checkbox>
            </Card>
        </FloatButtonGroup>
    </div>
</template>

<script setup lang="ts">
import { h, onMounted, ref, watch } from 'vue';
import { Button, Card, Checkbox, FloatButtonGroup, Image } from 'ant-design-vue';
import { DownloadOutlined } from '@ant-design/icons-vue';
import { ImagesType, ImgSave } from '@/utility/ImgSave';
import { WebDom } from '@/utility/WebDom';
const empty = "data:image/png;base64,";
const prop = defineProps<{ data: string }>();
const imgs = ref<ImagesType[]>([]);
const isSelect = ref(false)
const allSelect = ref(false)

function onSave() {
    ImgSave.imgTagToFile(imgs.value.filter(item => item.select))
}

//全选监听
watch(allSelect, (bol) => {
    imgs.value.forEach(item => item.select = bol);
});
onMounted(() => {
    const imgArr = Array.of(...new WebDom(prop.data).dom.getElementsByTagName('img')).map(img => { return { src: img.src, select: false, node: ref(null) } });
    imgs.value = imgArr;
});
</script>